...
## 其他
* 使用h2datebase spring springmvc 忙一晚上终于还算弄出来了，与IDE无关，试试吧，以前就是提交下就完了，签出如果不放在WEB容器里的话是肯定不行。。Junit用的次数较少，还是没调出来，不过直接访问效果还是有的，困得不行脑袋木的就这样了，自己感觉写的都很一般，希望能在周一上班前给个结果，因为周一要入职了，如果能进银泰的话周一也就不去了
* 业务一：http://localhost:8080/test/json/张三/86.5/
* 业务二：http://localhost:8080/test/json/1/